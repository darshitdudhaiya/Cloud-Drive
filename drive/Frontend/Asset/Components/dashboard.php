...
include(pathOf('Backend/Query/Connection.php'));
...
<head>
  ...
  <link rel="stylesheet" href="<?= urlOf("Frontend/Asset/CSS/styles.css") ?>">
</head>
...
<div class="inline-flex h-full w-screen">
      ...
      <div class="max-w-xs">
        <div class="py-3">
          <div class="photo-wrapper p-2 cursor-pointer user-profile">
            <img class="w-32 h-32 rounded-full mx-auto" src="<?= urlOf('Frontend/Asset/Images/download.jfif') ?>" alt="John Doe">
          </div>
          <div class="p-2">
            <h3 class="text-center text-xl text-gray-900 font-medium leading-8"><?= $uname?></h3>
          </div>
        </div>
      </div>
    </div>
  ...
  <!-- Main contant  -->
  <div class="mx-24 my-10 lg:mx-5 lg:my-6 lg:w-full lg:overflow-x-hidden lg:overflow-y-auto scroll-bar">
    <div class="grid grid-cols-2 lg:grid-cols-5 xxl:grid-cols-7 gap-5 lg:gap-x-0 lg:gap-y-3">
      <?php
      for ($i = 0; $i < 20; $i++) {
        echo '
    ...
    <div id="logout-modal" tabindex="-1" class="absolute hidden justify-center  z-50 pt-40 p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
  ...
  <script src="<?=urlOf('Frontend/Asset/Js/index.js')?>"></script>
  </body>
